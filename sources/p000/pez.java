package p000;

import android.os.Parcel;
import android.os.Parcelable;
import com.google.android.gms.cast.EqualizerBandSettings;
import com.google.android.gms.cast.EqualizerSettings;

/* renamed from: pez */
/* compiled from: :com.google.android.gms@201515033@20.15.15 (120300-306758586) */
public final class pez implements Parcelable.Creator {
    public final /* bridge */ /* synthetic */ Object[] newArray(int i) {
        return new EqualizerSettings[i];
    }

    public final /* bridge */ /* synthetic */ Object createFromParcel(Parcel parcel) {
        int b = sed.m35001b(parcel);
        EqualizerBandSettings equalizerBandSettings = null;
        EqualizerBandSettings equalizerBandSettings2 = null;
        while (parcel.dataPosition() < b) {
            int readInt = parcel.readInt();
            int a = sed.m34995a(readInt);
            if (a == 2) {
                equalizerBandSettings = (EqualizerBandSettings) sed.m34998a(parcel, readInt, EqualizerBandSettings.CREATOR);
            } else if (a != 3) {
                sed.m35002b(parcel, readInt);
            } else {
                equalizerBandSettings2 = (EqualizerBandSettings) sed.m34998a(parcel, readInt, EqualizerBandSettings.CREATOR);
            }
        }
        sed.m34994F(parcel, b);
        return new EqualizerSettings(equalizerBandSettings, equalizerBandSettings2);
    }
}
