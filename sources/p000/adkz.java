package p000;

/* renamed from: adkz */
/* compiled from: :com.google.android.gms@201515033@20.15.15 (120300-306758586) */
public final class adkz {

    /* renamed from: a */
    private final bxvd f62097a = bkjv.f124474b.mo74144da();

    /* renamed from: a */
    public final void mo33627a(int i) {
        bxvd da = bkjw.f124477d.mo74144da();
        if (da.f164950c) {
            da.mo74035c();
            da.f164950c = false;
        }
        bkjw bkjw = (bkjw) da.f164949b;
        bkjw.f124480b = i - 1;
        bkjw.f124479a |= 1;
        long currentTimeMillis = System.currentTimeMillis();
        if (da.f164950c) {
            da.mo74035c();
            da.f164950c = false;
        }
        bkjw bkjw2 = (bkjw) da.f164949b;
        bkjw2.f124479a |= 2;
        bkjw2.f124481c = currentTimeMillis;
        bkjw bkjw3 = (bkjw) da.mo74062i();
        bxvd bxvd = this.f62097a;
        if (bxvd.f164950c) {
            bxvd.mo74035c();
            bxvd.f164950c = false;
        }
        bkjv bkjv = (bkjv) bxvd.f164949b;
        bkjv bkjv2 = bkjv.f124474b;
        bkjw3.getClass();
        if (!bkjv.f124476a.mo73666a()) {
            bkjv.f124476a = GeneratedMessageLite.m124021a(bkjv.f124476a);
        }
        bkjv.f124476a.add(bkjw3);
    }

    /* renamed from: a */
    public final byte[] mo33628a() {
        return ((bkjv) this.f62097a.mo74062i()).serializeToBytes();
    }
}
