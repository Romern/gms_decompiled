package p000;

/* renamed from: bogj */
/* compiled from: :com.google.android.gms@201515033@20.15.15 (120300-306758586) */
public final class bogj extends bxvd implements bxxd {
    public bogj() {
        super(bohi.f133070z);
    }

    /* renamed from: a */
    public final void mo68908a(bogm bogm) {
        if (this.f164950c) {
            mo74035c();
            this.f164950c = false;
        }
        bohi bohi = (bohi) this.f164949b;
        bohi bohi2 = bohi.f133070z;
        bogm.getClass();
        bohi.mo68911a();
        bohi.f133085o.add(bogm);
    }

    /* renamed from: a */
    public final void mo68909a(Iterable iterable) {
        if (this.f164950c) {
            mo74035c();
            this.f164950c = false;
        }
        bohi bohi = (bohi) this.f164949b;
        bohi bohi2 = bohi.f133070z;
        if (!bohi.f133081k.mo73666a()) {
            bohi.f133081k = bxvk.m124021a(bohi.f133081k);
        }
        bxsy.m123078a(iterable, bohi.f133081k);
    }
}
