package p000;

/* renamed from: bjlb */
/* compiled from: :com.google.android.gms@201515033@20.15.15 (120300-306758586) */
public final class bjlb {
    /* renamed from: a */
    public static bjlc m103818a() {
        bjld bjld = new bjld();
        long a = bjld.mo65363a();
        long b = bjld.mo65364b();
        bxvf bxvf = (bxvf) bwog.f160437f.mo74144da();
        if (bxvf.f164950c) {
            bxvf.mo74035c();
            bxvf.f164950c = false;
        }
        bwog bwog = (bwog) bxvf.f164949b;
        bwog.f160439a |= 1;
        bwog.f160440b = b;
        int a2 = bwtp.f160982d.mo73900a();
        if (bxvf.f164950c) {
            bxvf.mo74035c();
            bxvf.f164950c = false;
        }
        bwog bwog2 = (bwog) bxvf.f164949b;
        bwog2.f160439a |= 16;
        bwog2.f160443e = a2;
        bxvj bxvj = bwtp.f160982d;
        bxvd da = bwtp.f160981c.mo74144da();
        if (da.f164950c) {
            da.mo74035c();
            da.f164950c = false;
        }
        bwtp bwtp = (bwtp) da.f164949b;
        bwtp.f160985b = 2;
        bwtp.f160984a |= 1;
        bxvf.mo74125a(bxvj, (bwtp) da.mo74062i());
        bwog bwog3 = (bwog) bxvf.mo74062i();
        bxvf bxvf2 = (bxvf) bwny.f160414k.mo74144da();
        if (bxvf2.f164950c) {
            bxvf2.mo74035c();
            bxvf2.f164950c = false;
        }
        bwny bwny = (bwny) bxvf2.f164949b;
        bwog3.getClass();
        bwny.f160417b = bwog3;
        bwny.f160416a |= 1;
        int a3 = bwto.f160977d.mo73900a();
        if (bxvf2.f164950c) {
            bxvf2.mo74035c();
            bxvf2.f164950c = false;
        }
        bwny bwny2 = (bwny) bxvf2.f164949b;
        bwny2.f160416a |= 8;
        bwny2.f160423h = a3;
        bxvj bxvj2 = bwto.f160977d;
        bxvd da2 = bwto.f160976c.mo74144da();
        bxvd da3 = bwtj.f160961f.mo74144da();
        if (da3.f164950c) {
            da3.mo74035c();
            da3.f164950c = false;
        }
        bwtj bwtj = (bwtj) da3.f164949b;
        bwtj.f160967e = 1;
        bwtj.f160963a |= 2;
        bwtj bwtj2 = (bwtj) da3.mo74062i();
        if (da2.f164950c) {
            da2.mo74035c();
            da2.f164950c = false;
        }
        bwto bwto = (bwto) da2.f164949b;
        bwtj2.getClass();
        bwto.f160980b = bwtj2;
        bwto.f160979a |= 1;
        bxvf2.mo74125a(bxvj2, (bwto) da2.mo74062i());
        bwny bwny3 = (bwny) bxvf2.mo74062i();
        bxvd da4 = bwxy.f161427d.mo74144da();
        if (da4.f164950c) {
            da4.mo74035c();
            da4.f164950c = false;
        }
        bwxy bwxy = (bwxy) da4.f164949b;
        bwxy.f161430b = 1;
        bwxy.f161429a |= 1;
        bxvd da5 = bwya.f161433d.mo74144da();
        if (da5.f164950c) {
            da5.mo74035c();
            da5.f164950c = false;
        }
        bwya bwya = (bwya) da5.f164949b;
        bwya.f161436b = 1;
        int i = bwya.f161435a | 1;
        bwya.f161435a = i;
        bwya.f161435a = i | 2;
        bwya.f161437c = 48.0f;
        bwya bwya2 = (bwya) da5.mo74062i();
        if (da4.f164950c) {
            da4.mo74035c();
            da4.f164950c = false;
        }
        bwxy bwxy2 = (bwxy) da4.f164949b;
        bwya2.getClass();
        bwxy2.f161431c = bwya2;
        bwxy2.f161429a |= 2;
        bwxy bwxy3 = (bwxy) da4.mo74062i();
        bxvf bxvf3 = (bxvf) bwxl.f161383i.mo74144da();
        if (bxvf3.f164950c) {
            bxvf3.mo74035c();
            bxvf3.f164950c = false;
        }
        bwxl bwxl = (bwxl) bxvf3.f164949b;
        int i2 = bwxl.f161385a | 1;
        bwxl.f161385a = i2;
        bwxl.f161386b = a;
        bwxl.f161385a = i2 | 4;
        bwxl.f161388d = b;
        int a4 = bwtq.f160987d.mo73900a();
        if (bxvf3.f164950c) {
            bxvf3.mo74035c();
            bxvf3.f164950c = false;
        }
        bwxl bwxl2 = (bwxl) bxvf3.f164949b;
        bwxl2.f161385a |= 16;
        bwxl2.f161390f = a4;
        bxvj bxvj3 = bwtq.f160987d;
        bxvd da6 = bwtq.f160986c.mo74144da();
        bxvd da7 = bwtt.f160992e.mo74144da();
        if (da7.f164950c) {
            da7.mo74035c();
            da7.f164950c = false;
        }
        bwtt bwtt = (bwtt) da7.f164949b;
        bwtt.f160995b = 1;
        bwtt.f160994a |= 1;
        if (da6.f164950c) {
            da6.mo74035c();
            da6.f164950c = false;
        }
        bwtq bwtq = (bwtq) da6.f164949b;
        bwtt bwtt2 = (bwtt) da7.mo74062i();
        bwtt2.getClass();
        bwtq.f160990b = bwtt2;
        bwtq.f160989a = 1;
        bxvf3.mo74125a(bxvj3, (bwtq) da6.mo74062i());
        bwxr bwxr = (bwxr) bwxs.f161407d.mo74144da();
        bxvd da8 = bwxq.f161401f.mo74144da();
        bxvd da9 = bwxv.f161421d.mo74144da();
        bxvd da10 = bwxj.f161360q.mo74144da();
        if (da10.f164950c) {
            da10.mo74035c();
            da10.f164950c = false;
        }
        bwxj bwxj = (bwxj) da10.f164949b;
        bwxy3.getClass();
        bwxj.f161372k = bwxy3;
        int i3 = bwxj.f161362a | 32;
        bwxj.f161362a = i3;
        bwxy3.getClass();
        bwxj.f161369h = bwxy3;
        bwxj.f161362a = i3 | 4;
        bxvd da11 = bwxc.f161321f.mo74144da();
        bxvd da12 = bwya.f161433d.mo74144da();
        if (da12.f164950c) {
            da12.mo74035c();
            da12.f164950c = false;
        }
        bwya bwya3 = (bwya) da12.f164949b;
        bwya3.f161436b = 1;
        int i4 = bwya3.f161435a | 1;
        bwya3.f161435a = i4;
        bwya3.f161435a = i4 | 2;
        bwya3.f161437c = 96.0f;
        if (da11.f164950c) {
            da11.mo74035c();
            da11.f164950c = false;
        }
        bwxc bwxc = (bwxc) da11.f164949b;
        bwya bwya4 = (bwya) da12.mo74062i();
        bwya4.getClass();
        bwxc.f161324b = bwya4;
        bwxc.f161323a |= 1;
        if (da10.f164950c) {
            da10.mo74035c();
            da10.f164950c = false;
        }
        bwxj bwxj2 = (bwxj) da10.f164949b;
        bwxc bwxc2 = (bwxc) da11.mo74062i();
        bwxc2.getClass();
        bwxj2.f161367f = bwxc2;
        bwxj2.f161362a |= 1;
        bxvd da13 = bwwz.f161308d.mo74144da();
        bwwy bwwy = bwwy.ALIGNMENT_CENTER;
        if (da13.f164950c) {
            da13.mo74035c();
            da13.f164950c = false;
        }
        bwwz bwwz = (bwwz) da13.f164949b;
        bwwz.f161312c = bwwy.f161307i;
        bwwz.f161310a |= 2;
        if (da10.f164950c) {
            da10.mo74035c();
            da10.f164950c = false;
        }
        bwxj bwxj3 = (bwxj) da10.f164949b;
        bwwz bwwz2 = (bwwz) da13.mo74062i();
        bwwz2.getClass();
        bwxj3.f161376o = bwwz2;
        bwxj3.f161362a |= 512;
        if (da9.f164950c) {
            da9.mo74035c();
            da9.f164950c = false;
        }
        bwxv bwxv = (bwxv) da9.f164949b;
        bwxj bwxj4 = (bwxj) da10.mo74062i();
        bwxj4.getClass();
        bwxv.f161424b = bwxj4;
        bwxv.f161423a |= 1;
        if (da8.f164950c) {
            da8.mo74035c();
            da8.f164950c = false;
        }
        bwxq bwxq = (bwxq) da8.f164949b;
        bwxv bwxv2 = (bwxv) da9.mo74062i();
        bwxv2.getClass();
        bwxq.f161406e = bwxv2;
        bwxq.f161403a |= 1;
        bwxr.mo73538a(da8);
        if (bxvf3.f164950c) {
            bxvf3.mo74035c();
            bxvf3.f164950c = false;
        }
        bwxl bwxl3 = (bwxl) bxvf3.f164949b;
        bwxs bwxs = (bwxs) bwxr.mo74062i();
        bwxs.getClass();
        bwxl3.f161387c = bwxs;
        bwxl3.f161385a |= 2;
        bwxl bwxl4 = (bwxl) bxvf3.mo74062i();
        long a5 = bjld.mo65363a();
        long b2 = bjld.mo65364b();
        bxvf bxvf4 = (bxvf) bwog.f160437f.mo74144da();
        if (bxvf4.f164950c) {
            bxvf4.mo74035c();
            bxvf4.f164950c = false;
        }
        bwog bwog4 = (bwog) bxvf4.f164949b;
        bwog4.f160439a |= 1;
        bwog4.f160440b = b2;
        bwog bwog5 = (bwog) bxvf4.mo74062i();
        bxvf bxvf5 = (bxvf) bwny.f160414k.mo74144da();
        if (bxvf5.f164950c) {
            bxvf5.mo74035c();
            bxvf5.f164950c = false;
        }
        bwny bwny4 = (bwny) bxvf5.f164949b;
        bwog5.getClass();
        bwny4.f160417b = bwog5;
        bwny4.f160416a |= 1;
        int a6 = bwsx.f160924c.mo73900a();
        if (bxvf5.f164950c) {
            bxvf5.mo74035c();
            bxvf5.f164950c = false;
        }
        bwny bwny5 = (bwny) bxvf5.f164949b;
        bwny5.f160416a |= 8;
        bwny5.f160423h = a6;
        bxvj bxvj4 = bwsx.f160924c;
        bwsw bwsw = (bwsw) bwsx.f160923b.mo74144da();
        bwsw.mo73533a(b);
        bxvf5.mo74125a(bxvj4, (bwsx) bwsw.mo74062i());
        bwny bwny6 = (bwny) bxvf5.mo74062i();
        bxvf bxvf6 = (bxvf) bwxl.f161383i.mo74144da();
        if (bxvf6.f164950c) {
            bxvf6.mo74035c();
            bxvf6.f164950c = false;
        }
        bwxl bwxl5 = (bwxl) bxvf6.f164949b;
        int i5 = bwxl5.f161385a | 1;
        bwxl5.f161385a = i5;
        bwxl5.f161386b = a5;
        bwxl5.f161385a = i5 | 4;
        bwxl5.f161388d = b2;
        int a7 = bwsv.f160917e.mo73900a();
        if (bxvf6.f164950c) {
            bxvf6.mo74035c();
            bxvf6.f164950c = false;
        }
        bwxl bwxl6 = (bwxl) bxvf6.f164949b;
        bwxl6.f161385a |= 16;
        bwxl6.f161390f = a7;
        bxvj bxvj5 = bwsv.f160917e;
        bwss bwss = (bwss) bwsv.f160916d.mo74144da();
        bwss.mo73531a(bwxl4);
        if (bwss.f164950c) {
            bwss.mo74035c();
            bwss.f164950c = false;
        }
        bwsv bwsv = (bwsv) bwss.f164949b;
        bwsv.f160921c = 1;
        bwsv.f160919a |= 1;
        bxvf6.mo74125a(bxvj5, (bwsv) bwss.mo74062i());
        bwxr bwxr2 = (bwxr) bwxs.f161407d.mo74144da();
        bxvd da14 = bwxq.f161401f.mo74144da();
        bxvd da15 = bwxv.f161421d.mo74144da();
        bxvd da16 = bwxj.f161360q.mo74144da();
        bxvd da17 = bwxy.f161427d.mo74144da();
        if (da17.f164950c) {
            da17.mo74035c();
            da17.f164950c = false;
        }
        bwxy bwxy4 = (bwxy) da17.f164949b;
        bwxy4.f161430b = 2;
        bwxy4.f161429a |= 1;
        if (da16.f164950c) {
            da16.mo74035c();
            da16.f164950c = false;
        }
        bwxj bwxj5 = (bwxj) da16.f164949b;
        bwxy bwxy5 = (bwxy) da17.mo74062i();
        bwxy5.getClass();
        bwxj5.f161372k = bwxy5;
        bwxj5.f161362a |= 32;
        if (da15.f164950c) {
            da15.mo74035c();
            da15.f164950c = false;
        }
        bwxv bwxv3 = (bwxv) da15.f164949b;
        bwxj bwxj6 = (bwxj) da16.mo74062i();
        bwxj6.getClass();
        bwxv3.f161424b = bwxj6;
        bwxv3.f161423a |= 1;
        if (da14.f164950c) {
            da14.mo74035c();
            da14.f164950c = false;
        }
        bwxq bwxq2 = (bwxq) da14.f164949b;
        bwxv bwxv4 = (bwxv) da15.mo74062i();
        bwxv4.getClass();
        bwxq2.f161406e = bwxv4;
        bwxq2.f161403a |= 1;
        bwxr2.mo73538a(da14);
        if (bxvf6.f164950c) {
            bxvf6.mo74035c();
            bxvf6.f164950c = false;
        }
        bwxl bwxl7 = (bwxl) bxvf6.f164949b;
        bwxs bwxs2 = (bwxs) bwxr2.mo74062i();
        bwxs2.getClass();
        bwxl7.f161387c = bwxs2;
        bwxl7.f161385a |= 2;
        bwxl bwxl8 = (bwxl) bxvf6.mo74062i();
        long a8 = bjld.mo65363a();
        long b3 = bjld.mo65364b();
        long j = bjld.f122920a;
        long j2 = a8;
        bjld.f122920a = j - 1;
        int i6 = bjld.f122921b;
        bjld.f122921b = i6 - 1;
        int i7 = bjld.f122922c;
        bjld.f122922c = i7 - 1;
        bxvf bxvf7 = (bxvf) bwog.f160437f.mo74144da();
        if (bxvf7.f164950c) {
            bxvf7.mo74035c();
            bxvf7.f164950c = false;
        }
        bwog bwog6 = (bwog) bxvf7.f164949b;
        long j3 = a;
        bwog6.f160439a |= 1;
        bwog6.f160440b = b3;
        int a9 = bwmr.f160301f.mo73900a();
        if (bxvf7.f164950c) {
            bxvf7.mo74035c();
            bxvf7.f164950c = false;
        }
        bwog bwog7 = (bwog) bxvf7.f164949b;
        bwog7.f160439a |= 16;
        bwog7.f160443e = a9;
        bxvj bxvj6 = bwmr.f160301f;
        bwmq bwmq = (bwmq) bwmr.f160300e.mo74144da();
        bwmu bwmu = bwmu.BOTTOM_SHEET_POSITION_STATE_COLLAPSED;
        long j4 = a5;
        if (bwmq.f164950c) {
            bwmq.mo74035c();
            bwmq.f164950c = false;
        }
        bwmr bwmr = (bwmr) bwmq.f164949b;
        bwmr.f160304b = bwmu.f160321f;
        bwmr.f160303a |= 1;
        bwmq.mo73528a(bwmu.BOTTOM_SHEET_POSITION_STATE_COLLAPSED);
        bwmq.mo73528a(bwmu.BOTTOM_SHEET_POSITION_STATE_HIDDEN);
        bxvf7.mo74125a(bxvj6, (bwmr) bwmq.mo74062i());
        bwog bwog8 = (bwog) bxvf7.mo74062i();
        bxvf bxvf8 = (bxvf) bwny.f160414k.mo74144da();
        if (bxvf8.f164950c) {
            bxvf8.mo74035c();
            bxvf8.f164950c = false;
        }
        bwny bwny7 = (bwny) bxvf8.f164949b;
        bwog8.getClass();
        bwny7.f160417b = bwog8;
        bwny7.f160416a |= 1;
        int a10 = bwmo.f160297b.mo73900a();
        if (bxvf8.f164950c) {
            bxvf8.mo74035c();
            bxvf8.f164950c = false;
        }
        bwny bwny8 = (bwny) bxvf8.f164949b;
        bwny8.f160416a |= 8;
        bwny8.f160423h = a10;
        bxvf8.mo74125a(bwmo.f160297b, bwmo.f160296a);
        bxvf bxvf9 = (bxvf) bwou.f160492e.mo74144da();
        if (bxvf9.f164950c) {
            bxvf9.mo74035c();
            bxvf9.f164950c = false;
        }
        bwou bwou = (bwou) bxvf9.f164949b;
        bwou.f160494a |= 1;
        bwou.f160496c = i6;
        if (!bwou.f160495b.mo73666a()) {
            bwou.f160495b = GeneratedMessageLite.m124020a(bwou.f160495b);
        }
        bwou.f160495b.mo74161a(j);
        int a11 = bwov.f160500b.mo73900a();
        if (bxvf9.f164950c) {
            bxvf9.mo74035c();
            bxvf9.f164950c = false;
        }
        bwou bwou2 = (bwou) bxvf9.f164949b;
        bwou2.f160494a |= 2;
        bwou2.f160497d = a11;
        bxvf9.mo74125a(bwov.f160500b, bwov.f160499a);
        if (bxvf8.f164950c) {
            bxvf8.mo74035c();
            bxvf8.f164950c = false;
        }
        bwny bwny9 = (bwny) bxvf8.f164949b;
        bwou bwou3 = (bwou) bxvf9.mo74062i();
        bwou3.getClass();
        if (!bwny9.f160420e.mo73666a()) {
            bwny9.f160420e = GeneratedMessageLite.m124021a(bwny9.f160420e);
        }
        bwny9.f160420e.add(bwou3);
        bxvf bxvf10 = (bxvf) bwoi.f160451f.mo74144da();
        if (bxvf10.f164950c) {
            bxvf10.mo74035c();
            bxvf10.f164950c = false;
        }
        bwoi bwoi = (bwoi) bxvf10.f164949b;
        bwoi.f160453a |= 2;
        bwoi.f160456d = i7;
        if (!bwoi.f160454b.mo73666a()) {
            bwoi.f160454b = GeneratedMessageLite.m124020a(bwoi.f160454b);
        }
        bwoi.f160454b.mo74161a(j);
        int a12 = bwmn.f160290f.mo73900a();
        if (bxvf10.f164950c) {
            bxvf10.mo74035c();
            bxvf10.f164950c = false;
        }
        bwoi bwoi2 = (bwoi) bxvf10.f164949b;
        bwoi2.f160453a |= 1;
        bwoi2.f160455c = a12;
        bxvj bxvj7 = bwmn.f160290f;
        bxvd da18 = bwmn.f160289e.mo74144da();
        if (da18.f164950c) {
            da18.mo74035c();
            da18.f164950c = false;
        }
        bwmn bwmn = (bwmn) da18.f164949b;
        bwmn.f160295d = 1;
        bwmn.f160292a |= 1;
        bwmu bwmu2 = bwmu.BOTTOM_SHEET_POSITION_STATE_HIDDEN;
        if (da18.f164950c) {
            da18.mo74035c();
            da18.f164950c = false;
        }
        bwmn bwmn2 = (bwmn) da18.f164949b;
        bwmn2.f160294c = Integer.valueOf(bwmu2.f160321f);
        bwmn2.f160293b = 2;
        bxvf10.mo74125a(bxvj7, (bwmn) da18.mo74062i());
        if (bxvf8.f164950c) {
            bxvf8.mo74035c();
            bxvf8.f164950c = false;
        }
        bwny bwny10 = (bwny) bxvf8.f164949b;
        bwoi bwoi3 = (bwoi) bxvf10.mo74062i();
        bwoi3.getClass();
        if (!bwny10.f160421f.mo73666a()) {
            bwny10.f160421f = GeneratedMessageLite.m124021a(bwny10.f160421f);
        }
        bwny10.f160421f.add(bwoi3);
        bwny bwny11 = (bwny) bxvf8.mo74062i();
        bxvf bxvf11 = (bxvf) bwxl.f161383i.mo74144da();
        if (bxvf11.f164950c) {
            bxvf11.mo74035c();
            bxvf11.f164950c = false;
        }
        bwxl bwxl9 = (bwxl) bxvf11.f164949b;
        int i8 = bwxl9.f161385a | 1;
        bwxl9.f161385a = i8;
        long j5 = j2;
        bwxl9.f161386b = j5;
        bwxl9.f161385a = i8 | 4;
        bwxl9.f161388d = b3;
        int a13 = bwms.f160307f.mo73900a();
        if (bxvf11.f164950c) {
            bxvf11.mo74035c();
            bxvf11.f164950c = false;
        }
        bwxl bwxl10 = (bwxl) bxvf11.f164949b;
        bwxl10.f161385a |= 16;
        bwxl10.f161390f = a13;
        bxvj bxvj8 = bwms.f160307f;
        bxvd da19 = bwms.f160306e.mo74144da();
        if (da19.f164950c) {
            da19.mo74035c();
            da19.f164950c = false;
        }
        bwms bwms = (bwms) da19.f164949b;
        int i9 = bwms.f160309a | 8;
        bwms.f160309a = i9;
        bwms.f160312d = 33;
        int i10 = i9 | 4;
        bwms.f160309a = i10;
        bwms.f160311c = 33;
        bwxl8.getClass();
        bwms.f160310b = bwxl8;
        bwms.f160309a = i10 | 1;
        bxvf11.mo74125a(bxvj8, (bwms) da19.mo74062i());
        bwxl bwxl11 = (bwxl) bxvf11.mo74062i();
        long b4 = bjld.mo65364b();
        bxvf bxvf12 = (bxvf) bwog.f160437f.mo74144da();
        if (bxvf12.f164950c) {
            bxvf12.mo74035c();
            bxvf12.f164950c = false;
        }
        bwog bwog9 = (bwog) bxvf12.f164949b;
        bwog9.f160439a |= 1;
        bwog9.f160440b = b4;
        bwog bwog10 = (bwog) bxvf12.mo74062i();
        bxvf bxvf13 = (bxvf) bwny.f160414k.mo74144da();
        if (bxvf13.f164950c) {
            bxvf13.mo74035c();
            bxvf13.f164950c = false;
        }
        bwny bwny12 = (bwny) bxvf13.f164949b;
        bwog10.getClass();
        bwny12.f160417b = bwog10;
        bwny12.f160416a |= 1;
        int a14 = bwqt.f160722c.mo73900a();
        if (bxvf13.f164950c) {
            bxvf13.mo74035c();
            bxvf13.f164950c = false;
        }
        bwny bwny13 = (bwny) bxvf13.f164949b;
        bwny13.f160416a |= 8;
        bwny13.f160423h = a14;
        bxvf13.mo74125a(bwqt.f160722c, bwqt.f160721b);
        bxvf bxvf14 = (bxvf) bwoq.f160480d.mo74144da();
        if (bxvf14.f164950c) {
            bxvf14.mo74035c();
            bxvf14.f164950c = false;
        }
        bwoq bwoq = (bwoq) bxvf14.f164949b;
        if (!bwoq.f160483b.mo73666a()) {
            bwoq.f160483b = GeneratedMessageLite.m124020a(bwoq.f160483b);
        }
        bwoq.f160483b.mo74161a(j);
        int a15 = bwrf.f160755f.mo73900a();
        if (bxvf14.f164950c) {
            bxvf14.mo74035c();
            bxvf14.f164950c = false;
        }
        bwoq bwoq2 = (bwoq) bxvf14.f164949b;
        bwoq2.f160482a |= 1;
        bwoq2.f160484c = a15;
        bxvj bxvj9 = bwrf.f160755f;
        bxvd da20 = bwrf.f160754e.mo74144da();
        if (da20.f164950c) {
            da20.mo74035c();
            da20.f164950c = false;
        }
        bwrf bwrf = (bwrf) da20.f164949b;
        bwrf.f160760d = 2;
        bwrf.f160757a |= 1;
        bxvd da21 = bwrc.f160746e.mo74144da();
        if (da21.f164950c) {
            da21.mo74035c();
            da21.f164950c = false;
        }
        bwrc bwrc = (bwrc) da21.f164949b;
        bwrc.f160749b = 2;
        bwrc.f160748a |= 1;
        if (da20.f164950c) {
            da20.mo74035c();
            da20.f164950c = false;
        }
        bwrf bwrf2 = (bwrf) da20.f164949b;
        bwrc bwrc2 = (bwrc) da21.mo74062i();
        bwrc2.getClass();
        bwrf2.f160759c = bwrc2;
        bwrf2.f160758b = 3;
        bxvf14.mo74125a(bxvj9, (bwrf) da20.mo74062i());
        if (bxvf13.f164950c) {
            bxvf13.mo74035c();
            bxvf13.f164950c = false;
        }
        bwny bwny14 = (bwny) bxvf13.f164949b;
        bwoq bwoq3 = (bwoq) bxvf14.mo74062i();
        bwoq3.getClass();
        if (!bwny14.f160422g.mo73666a()) {
            bwny14.f160422g = GeneratedMessageLite.m124021a(bwny14.f160422g);
        }
        bwny14.f160422g.add(bwoq3);
        bwny bwny15 = (bwny) bxvf13.mo74062i();
        bwyy bwyy = (bwyy) bwyz.f161561h.mo74144da();
        if (bwyy.f164950c) {
            bwyy.mo74035c();
            bwyy.f164950c = false;
        }
        bwyz bwyz = (bwyz) bwyy.f164949b;
        bwxl11.getClass();
        bwyz.f161564b = bwxl11;
        bwyz.f161563a |= 1;
        bwyy.mo73545a(bwny11);
        bwyy.mo73545a(bwny6);
        bwyy.mo73545a(bwny3);
        bwyy.mo73545a(bwny15);
        bwyz bwyz2 = (bwyz) bwyy.mo74062i();
        bxvd da22 = bwyw.f161544e.mo74144da();
        if (da22.f164950c) {
            da22.mo74035c();
            da22.f164950c = false;
        }
        bwyw bwyw = (bwyw) da22.f164949b;
        bwxl11.getClass();
        bwyw.f161549d = bwxl11;
        bwyw.f161546a |= 1;
        bxvd da23 = bwyv.f161533i.mo74144da();
        if (da23.f164950c) {
            da23.mo74035c();
            da23.f164950c = false;
        }
        bwyv bwyv = (bwyv) da23.f164949b;
        int i11 = bwyv.f161535a | 1;
        bwyv.f161535a = i11;
        bwyv.f161536b = 1;
        int i12 = 2 | i11;
        bwyv.f161535a = i12;
        bwyv.f161537c = j5;
        bwny11.getClass();
        bwyv.f161538d = bwny11;
        int i13 = i12 | 4;
        bwyv.f161535a = i13;
        int i14 = i13 | 8;
        bwyv.f161535a = i14;
        bwyv.f161539e = j4;
        bwny6.getClass();
        bwyv.f161540f = bwny6;
        int i15 = i14 | 16;
        bwyv.f161535a = i15;
        int i16 = i15 | 32;
        bwyv.f161535a = i16;
        bwyv.f161541g = j3;
        bwny3.getClass();
        bwyv.f161542h = bwny3;
        bwyv.f161535a = i16 | 64;
        if (da22.f164950c) {
            da22.mo74035c();
            da22.f164950c = false;
        }
        bwyw bwyw2 = (bwyw) da22.f164949b;
        bwyv bwyv2 = (bwyv) da23.mo74062i();
        bwyv2.getClass();
        bwyw2.f161548c = bwyv2;
        bwyw2.f161547b = 3;
        return new bjlc(bwyz2, (bwyw) da22.mo74062i());
    }
}
