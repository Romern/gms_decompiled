package p000;

/* renamed from: ammr */
/* compiled from: :com.google.android.gms@201515033@20.15.15 (120300-306758586) */
public final class ammr extends bxvd implements bxxd {
    public ammr() {
        super(amnu.f75540w);
    }

    /* renamed from: a */
    public final void mo41217a(amno amno) {
        if (this.f164950c) {
            mo74035c();
            this.f164950c = false;
        }
        amnu amnu = (amnu) this.f164949b;
        amnu amnu2 = amnu.f75540w;
        amno.getClass();
        if (!amnu.f75562u.mo73666a()) {
            amnu.f75562u = bxvk.m124021a(amnu.f75562u);
        }
        amnu.f75562u.add(amno);
    }

    /* renamed from: a */
    public final void mo41218a(amnr amnr) {
        if (this.f164950c) {
            mo74035c();
            this.f164950c = false;
        }
        amnu amnu = (amnu) this.f164949b;
        amnu amnu2 = amnu.f75540w;
        amnr.getClass();
        if (!amnu.f75561t.mo73666a()) {
            amnu.f75561t = bxvk.m124021a(amnu.f75561t);
        }
        amnu.f75561t.add(amnr);
    }
}
